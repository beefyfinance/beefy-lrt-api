...
export default async function (
...
) {
  {
    ...
    const urlParamsSchema = S.object()
      ...
      .prop(
        "vaultAddress",
        addressSchema.required().description("The vault address")
      );
...
export const getRingsRows = async (block: bigint, vaultAddress: Hex) => {
  ...
  const configs = await getBeefyVaultConfig(
    ...
    (v) =>
      ...
      (v.protocol_type === "beefy_clm_vault" &&
        v.beefy_clm_manager.vault_address.toLowerCase() ===
  ...
  const balances = await asyncCache.wrap(
    `rings:${chain}:breakdown:${block}:${vaultAddress}`,
    30_000,
    () =>
  ...
  const totalAmount = Object.values(amountPerHolder).reduce(
    (acc, b) => acc + b,
    0n
  );
