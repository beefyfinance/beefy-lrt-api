...
export default async function (
...
) {
  {
    ...
    instance.get<{ Params: UrlParams }>(
      "/hyperevm/:block/shares",
      { schema },
      async (request, reply) => {
        const { block } = request.params;
        const res = await getKittenswapRows(BigInt(block));
        if (!res) {
          reply.status(404);
...
export const getKittenswapRows = async (block: bigint) => {
  ...
  const configs = await getBeefyVaultConfig(
    chain,
    (v) =>
      v.pointStructureIds.includes("kittenswap-points") ||
